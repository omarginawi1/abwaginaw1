...
if (!empty($get_video->rent_price) && $get_video->rent_price > 0) {
    if (empty($user->is_pro) || $user->is_pro == 0) {
...
$user_is_pro = (!empty($pt->user) && $pt->user->is_pro == 1);
if ($user_is_pro) {
    $pt->video_approved = true;
}

if ($pt->config->approve_videos == 'on' || ($pt->config->auto_approve_ == 'no' && ($get_video->sell_video || $get_video->rent_price))) {
    if ($get_video->approved == 0 && !PT_IsAdmin() && (!isset($user->is_pro) || $user->is_pro != 1)) {
...
$user_is_pro = (!empty($pt->user) && $pt->user->is_pro == 1);
if (
  ($get_video->sell_video > 0 || $get_video->rent_price > 0)
  && !$user_is_pro
) {
    if (!empty($user->id)) {
...
// ✅ حماية إضافية: إذا الفيديو خصوصيته pro_only وما مشترك في الباقة
if ($get_video->privacy == 'pro_only' && (!IS_LOGGED || $pt->user->is_pro != 1)) {
